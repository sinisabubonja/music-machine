...
class Widget
{
  ...
  color inactiveColor = color(125, 157, 133);
  color activeColor = color(100, 100, 160);
  color bgColor = inactiveColor;
  color lineColor = color(125, 157, 133);
  
  PImage starfish=loadImage("starfish.png");
  PImage shell=loadImage("shell.png");
...
class Slider extends Widget
{
  ...
  void display()
  {
    ...
    if(orientation ==  HORIZONTAL){
        ...
        image(shell,pos.x+textW+2+sliderPos-extents.y/2,pos.y+2,extents.y-4,extents.y-4);
        ...
        //image(shell,200,200,30,30);//pos.x+2,pos.y+textW+2+sliderPos-maximum,extents.x-4,extents.x-4);
    } else if(orientation == UPWARDS){
        sliderPos = map(val, minimum, maximum, 0, extents.y-textW-4); 
        rect(pos.x+2, pos.y+textW+2 + (extents.y-textW-4-sliderPos), extents.x-4, sliderPos);
        image(starfish,pos.x+2,pos.y+textW+2 + (extents.y-textW-4-sliderPos)-extents.x/2,extents.x-4,extents.x-4);
  };
    popStyle();
  }
